...
Function WriteConfigFile
  ...
  ; Never "*": a wildcard exposes every API response to any site, and the
  ; redirection (KVM/SOL/IDER) relay refuses cross-origin handshakes while it is
  ; set. Add the UI origin here if the UI is not served by this server.
  FileWrite $1 "  allowed_origins:$\r$\n"
  FileWrite $1 "    - $\"https://localhost:$PortValue$\"$\r$\n"
  FileWrite $1 "    - $\"https://127.0.0.1:$PortValue$\"$\r$\n"
  FileWrite $1 "    - $\"http://localhost:$PortValue$\"$\r$\n"
  FileWrite $1 "    - $\"http://127.0.0.1:$PortValue$\"$\r$\n"
  FileWrite $1 "  allowed_headers:$\r$\n"
  FileWrite $1 "    - $\"Origin$\"$\r$\n"
  FileWrite $1 "    - $\"Accept$\"$\r$\n"
  FileWrite $1 "    - $\"Content-Type$\"$\r$\n"
  FileWrite $1 "    - $\"Content-Length$\"$\r$\n"
  FileWrite $1 "    - $\"Authorization$\"$\r$\n"
  FileWrite $1 "    - $\"If-Match$\"$\r$\n"
  FileWrite $1 "  allow_credentials: true$\r$\n"
