...
if [ -x "$UI_BINARY" ] && [ -x "$HEADLESS_BINARY" ]; then
    ...
    (cd "$CONSOLE_SRC" && CGO_ENABLED=1 GOOS=darwin GOARCH=$ARCH go build -tags=tray -ldflags "$LDFLAGS" -trimpath -o "$UI_BINARY" ./cmd/app)
    echo "  Built: $UI_BINARY"

    echo "Building headless binary with tray (CGO_ENABLED=1)..."
    (cd "$CONSOLE_SRC" && CGO_ENABLED=1 GOOS=darwin GOARCH=$ARCH go build -tags='tray noui' -ldflags "$LDFLAGS" -trimpath -o "$HEADLESS_BINARY" ./cmd/app)
...
build_pkg() {
    ...
    # Copy scripts. preinstall + postinstall are shared; build-time sed
    # bakes in the version and edition label.
    cp "$SCRIPT_DIR/scripts/preinstall" "$BUILD_DIR/scripts/"
    cp "$SCRIPT_DIR/scripts/postinstall" "$BUILD_DIR/scripts/"
    if [ "$EDITION" = "ui" ]; then
        EDITION_LABEL="UI"
    else
        EDITION_LABEL="Headless"
    fi
    sed -i '' "s/VERSION_PLACEHOLDER/$VERSION/g" "$BUILD_DIR/scripts/preinstall"
    sed -i '' \
        -e "s/VERSION_PLACEHOLDER/$VERSION/g" \
        -e "s/EDITION_LABEL_PLACEHOLDER/$EDITION_LABEL/g" \
    ...
    sed -e "s/VERSION_PLACEHOLDER/$VERSION/g" \
        -e "s/IDENTIFIER_PLACEHOLDER/$IDENTIFIER/g" \
        "$SCRIPT_DIR/distribution.xml" > "$BUILD_DIR/distribution.xml"
